...
static sgx_status_t SGX_CDECL sgx_ecall_init(void* pms)
{
	...
	ecall_init(ms->ms_stash_del_size, ms->ms_w_delete_token_size);


	return status;
}

static sgx_status_t SGX_CDECL sgx_ecall_scan(void* pms)
{
	sgx_status_t status = SGX_SUCCESS;
...
SGX_EXTERNC const struct {
	...
	3,
	{
		...
		{(void*)(uintptr_t)sgx_ecall_scan, 0},
		{(void*)(uintptr_t)sgx_ecall_reset, 0},
	}
};
